...
(defun session-dup ()
  ...
  (let ((name (my-desktop-get-current-name))
	(new_name (read-string "dup to:" (concat (my-desktop-get-current-name) "-" (format-time-string "%m-%d-%T"))))
	)
    (copy-directory (concat my-desktop-session-dir name) (concat my-desktop-session-dir new_name))
    (message new_name)
    )
  )
