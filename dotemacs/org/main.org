...
*** DONE Java Lightweight Lock
    ...
    Lightweight Lock 是java 1.6提供的一种锁优化机制, Dalvik 看起来也使用了这种机制. 在虚拟机中, synchronized 对应的具体指令是 MonitorEnter/MonitorExit, 例如:
#+BEGIN_HTML
...
下面的代码是 Dalvik 的 MonitorEnter() 的源码:
#+BEGIN_HTML
<pre lang="c" line="1">
    MonitorEnter:
...
#+END_HTML
    ...
    - spinlock
      ...
      spinlock适用于锁持有时间较短的情况,并且在SMP中,可以避免进程切换的开销 (不需要挂起), 但在单处理器了, 避免进程切换就无从谈起了.

    总结:
    - 因为Lightweight Lock机制, java的synchronized比想象的要高效, 特别是锁冲突较小的情况下,除了一条CAS指令,几乎没有额外开销.
    - Lightweight Lock过程
