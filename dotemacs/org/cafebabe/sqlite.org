...
** Other
*** WAL
Write Ahead Logging (WAL) 是与传统的 rollback journal (或 delete
journal, shadow paging) 完全不同的一种 journal mode. 通过 WAL, 数据库可
以做到读和写互相完全不干扰.

**** How WAL works
当 commit 一个 write transaction 时, rollback journal 的作法是先刷新所
有修改到 journal file, 然后再刷新 cache 到数据库. 而 WAL 的作法是只将修
改刷新到 wal journal file 中, 而不修改数据库. 只有当用户调用 pragma
wal_checkpoint 或 超过 1000 页被修改时, wal journal file 中的修改才会
被写回到数据库. 

为了使 read transaction 能读到最新的数据, 每个 read transaction 开始时
会扫描 wal journal file, 并在 wal file 中记下一个 mark, 在读数据库时,根
据一个 wal index shm 判断要读的 page 是否在 wal journal file 中, 若存在,
并且不超过 mark point, 则使用 wal journal file 中的数据, 否则使用数据库
中的数据. 这样可以使得读写不用互相等待. (但这样有一点问题, 就是无法保证
读到最新的数据, 因为有可能在读的过程中, 又有新的数据追加到 wal journal
file 中 makr point 之后)

WAL 的优点:
- 读写互不影响
- commit 不需要写太多的数据, 降低了IO的使用

WAL 的缺点:
- 可能无法读到"最新"的数据
- 过大的 read transaction 会阻止 checkpoint (当 read transaction 正在进
  行时, 虽然允许 write transaction 追加数据到 wal journal file, 但不允
  许清空, 因为 read transaction 已经在journal file 中标记了 mark
  point), 进而导致 wal journal file 过大, 从而使 read transaction 变慢
