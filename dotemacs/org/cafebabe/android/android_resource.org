...
- What indeed is `isMoreSpecificThan' ? How to choose the `MORE
  SPECIFIC' resource ? How to make decision between `mdpi' and `hdpi'
  drawable ? ...
- The priority between different configuration qualifiers?
  e.g. whether `drawable-mdpi' is more specific than
  `drawable-w720dp' ? since `mdpi' and `w720dp' is actually
  irrelevant.
