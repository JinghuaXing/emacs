...
CMS 可以更快, 是因为并发标记期间并没有 `stop the world`, 但带来的后果
是并发标记结束后有可能会因为其他线程的执行产生新的 garbage, 这时就需要
重新标记. 但是重新标记不能像初始标记和并发标记那样从头扫描一次, 这样就
速度太慢没有意义. 
  ...
  scanDirtyCards
    ...
    while (card < end):
      if (*card != GC_CARD_DIRTY):
        return card;
      while (ptr < limit):
        // 所谓的 gray object, 是否之间是可达的 (black reference)
        // 但现在有可能是不可达的 (white reference)
