...
*** classLookup
*** registerMap
register map 主要用来帮助 dvm 知道在每个函数的第个 GC Point 处当前各个寄存器中哪些保存着 java 对象.
以便进行 exact GC.

若没有这个 map, 那么 GC 时 dvm 只能假设当前所有寄存器中都保存着对象引
用, 从而导致一些本来已经是 garbage 的对象没有被回收. (例如 r1 本来保存
着一个整数 0xxxxx, 但这个整数刚好对应着某个已经不可达的对象的地址, 那
么这个对象就会被重新标记为可达)
